      ...
      <div className="space-y-4">
        {posts.slice(0, 10).map((post) => (
          ...
          <section>
            ...
            <CodeBlock
              code={`// 1. デフォルト: 自動キャッシュ（推奨）
// Next.js が自動的に最適なキャッシュを行う
const res = await fetch('https://api.example.com/data');

// 2. キャッシュしない（毎回最新データを取得）
// リアルタイム性が重要なデータに使う
const res2 = await fetch('https://api.example.com/realtime-data', {
  cache: 'no-store',
});
              ...
              <table className="w-full text-sm border-collapse">
                ...
                <tbody className="text-foreground/80">
                  <tr className="border-b border-border/50">
                    <td className="py-3 px-4 font-medium">デフォルト</td>
                    <td className="py-3 px-4">変更頻度の低いデータ</td>
                    <td className="py-3 px-4">会社概要、静的コンテンツ</td>
                  </tr>
                  <tr className="border-b border-border/50">
                    <td className="py-3 px-4 font-medium">no-store</td>
                    <td className="py-3 px-4">常に最新が必要</td>
            ...
            <div className="space-y-4">
              <div className="rounded-lg border border-border p-4">
                ...
                <p className="text-sm text-foreground/80">
                  ...
                  Next.js は同じ URL への fetch を自動的に重複排除（デデュプリケーション）するため、パフォーマンスの心配は不要です。
                </p>
              </div>
              <div className="rounded-lg border border-border p-4">
