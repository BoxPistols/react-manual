...
export default function CodingChallenge({
  ...
  const handleCheck = () => {
    if (validator) {
      ...
      // 簡易チェック: 空白と改行を正規化して比較
      const normalize = (s: string) => s.replace(/\s+/g, ' ').trim();
      setIsCorrect(normalize(code) === normalize(answer));
    }
  };
    ...
    <div className="rounded-xl border-2 border-emerald-200 dark:border-emerald-800 bg-emerald-50/50 dark:bg-emerald-950/20 p-6 my-6">
      ...
      <div className="rounded-lg overflow-hidden border border-border bg-[#1e1e2e] mb-4">
        ...
        <textarea
          ...
          spellCheck={false}
          className="w-full p-4 font-mono text-sm leading-relaxed bg-transparent text-[#cdd6f4] resize-none focus:outline-none min-h-[160px]"
      ...
      {showAnswer && (
        <div className="rounded-lg overflow-hidden border border-border bg-[#1e1e2e] mb-4">
          ...
          <pre className="p-4 font-mono text-sm leading-relaxed text-[#cdd6f4] overflow-x-auto">
            {answer}
          </pre>
        </div>
      )}
